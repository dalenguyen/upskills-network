...
import { toDashboardOrg } from './org-view';

/**
 * The dashboard org projection is the shape the organizer's own routes answer
 * with. The assertions here are about the wire format: the full roster is
 * present, and every Firestore `Timestamp` has become an ISO-8601 string before
 * it can be JSON-serialized.
 */

describe('toDashboardOrg', () => {
  it('serializes createdAt as an ISO-8601 string', () => {
    const view = toDashboardOrg(
      fakeOrg({
