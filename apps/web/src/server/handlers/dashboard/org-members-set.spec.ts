...
/** `POST` / `PUT /api/v1/dashboard/orgs/:orgId/members`. */
...
function deps(
  overrides: Partial<DashboardOrgMembersSetDeps> = {},
): DashboardOrgMembersSetDeps {
  return {
...
describe('POST /api/v1/dashboard/orgs/:orgId/members', () => {
  it('requires an org admin and adds or changes the member named by the body', async () => {
    const setOrgMember = vi.fn(async (): Promise<Organizer> => fakeOrg());
    const d = deps({ setOrgMember });
    const event = request({ uid: 'uid-2', role: 'manager' });

    const result = await createDashboardOrgMembersSetHandler(d)(event);
  ...
  it('answers 400 for a body that is not { uid, role }', async () => {
    ...
    await expect(
      createDashboardOrgMembersSetHandler(d)(request({ uid: 'uid-2' })),
