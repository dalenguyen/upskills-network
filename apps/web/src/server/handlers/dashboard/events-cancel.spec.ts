...
describe('DELETE /api/v1/dashboard/events/:eventId', () => {
  it('cancels the event and notifies every confirmed guest', async () => {
    ...
    expect(cancelEvent).toHaveBeenCalledWith('org-1', 'evt-1');
  ...
  it('serializes every timestamp as an ISO-8601 string', async () => {
    ...
    const result = (await createDashboardEventsCancelHandler(d)(
      request(),
    )) as Awaited<
      ReturnType<ReturnType<typeof createDashboardEventsCancelHandler>>
    >;
